...
def verify(images: np.ndarray, targets: np.ndarray) -> Tuple[int, int]:
    ## Implement
    tmp = model.forward(images)
    y = np.argmax(tmp, axis=1)
    # print(y)

    equals= y == targets
    num_ok = np.sum(equals)

    total_num = targets.size
    ## End
...
def test() -> float:
    ...
    for i in range(0, len(test_set), batch_size):
        ...
        accu2, count2 = verify(images, labels)
        accu += accu2
        count += count2
        ## End
...
for epoch in range(1000):
    ...
    for i in range(0, len(train_indices), batch_size):
        ...
        labels = [(np.zeros(10), l) for l in labels]
        for lab, val in labels:
            lab[val] = 1
        labels = [temp[0] for temp in labels]
        avg_error += lib.train_one_step(model, loss, learning_rate, images, labels) * len(labels)
    loss_value = avg_error / len(train_indices)
