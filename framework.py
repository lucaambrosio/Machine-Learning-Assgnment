...
class Softmax(Layer):
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        T = np.sum(np.exp(x), axis=1, keepdims=True)
        result = np.exp(x) / T
...
class Linear(Layer):
    ...
    def backward(self, grad_in: np.ndarray) -> Layer.BackwardResult:
        ...
        dW = np.transpose(x).dot(grad_in)
        db = grad_in.sum(axis=0)
        d_inputs = grad_in.dot(np.transpose(self.var["W"]))
        ## End
...
class CrossEntropy(Loss):
    def forward(self, prediction: np.ndarray, target: np.ndarray) -> float:
        ...
        self.saved_variables = {}
        self.saved_variables['prediction'] = prediction
        self.saved_variables['target'] = target
        self.saved_variables['input_size'] = n
    ...
    def backward(self) -> np.ndarray:
        ...
        n = self.saved_variables['input_size']
...
def train_one_step(model: Layer, loss: Loss, learning_rate: float, input: np.ndarray, target: np.ndarray) -> float:
    ...
    for k in model.var.keys():
        model.var[k] = model.var[k] - learning_rate * grad.variable_grads[k]
    ## End
    return loss_value
...
def gradient_check():
    ...
    for key, value in NN.var.items():
        ...
        for index in range(variable.shape[0]):
            ...
            mse = loss.forward(NN.forward(X), T)

            variable[index] = var_backup - eps
            mse_2 = loss.forward(NN.forward(X), T)

            numeric_grad = (mse - mse_2) / (2 * eps)
